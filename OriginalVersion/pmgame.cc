...
void GRAPHBOARD::OneUndo( HPS hps, PLAYER *Player )
{
	...
	{
		Move->DrawOutline( hps, Player );
		Move = Move->Undo( &Stones, Player );
		Move->GetNext()->Draw( hps );
	}
	Move->Fill( Stones );
	Move->DrawOutline( hps, Player );
	Move->Draw( hps );
...
BOOL GRAPHBOARD::Undo( HPS hps, PLAYER **Player )
{
	HOLE *HomePtr = ((*Player)->Side == TOP ? (HOLE*)TopHome : (HOLE*)BotHome);
	if( Moves[LastMove].Move == HomePtr ){	// if undo is called 
											// after the first
											// of two moves
		OneUndo( hps, *Player );	// move is taken back, same player's turn
		return TRUE;	// FirstMove is now true again
	} else
	{
		TogglePlayer( Player );
		OneUndo( hps, *Player );
		...
		if( Moves[LastMove].Move == HomePtr )
		{		// second move was taken back
			if( (*Player)->Kind == HUMAN )
			{
				return FALSE;		// it was the human's second move
			} else
			{			// take back computer's second move
				OneUndo( hps, *Player );
//				return Undo( hps, Player );	// Undo once again for the Human
			}
		}
		if( (*Player)->Kind == COMPUTER )
		{
			...
			return TRUE;		// it was the human's first move
		}
	}
}	
